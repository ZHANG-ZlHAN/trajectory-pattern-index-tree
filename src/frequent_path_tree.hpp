...
class FrequentPathTree {
public:
    explicit FrequentPathTree(
...
private:
    ...
    static HierarchicalBitmap make_bitmap(
    ...
    std::size_t missing_count(
        const HierarchicalBitmap& aggregate,
        const Entry& entry
    ) const {
    ...
    void search_recursive(
        const Node& node,
        const std::vector<std::size_t>& query,
        const HierarchicalBitmap& query_bitmap,
        Recommendation& best
    ) const {
        if (
